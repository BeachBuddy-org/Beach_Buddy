...
export const Gerente_Painel_CT = () => {
  ...
  const [treinadorData, setTreinadorData] = useState({
    username: "",
    email: "",
    cpf: "",
  });
  ...
  const handleAlunoSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://127.0.0.1:5000/api/register_aluno", {
        ...alunoData,
        ct_id: ctId,
      });
  ...
  const handleTreinadorSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://127.0.0.1:5000/api/register_treinador", {
        ...treinadorData,
        ct_id: ctId,
      });
  ...
  const handleTreinoSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("http://127.0.0.1:5000/api/create_treino", {
        ...treinoData,
        ct_id: ctId,
        horario,
      });
        ...
        <form onSubmit={handleTreinoSubmit}>
          ...
          <label>
            ...
            <input
              type="checkbox"
              name="recorrente"
              checked={treinoData.recorrente}
              onChange={(e) =>
                setTreinoData({ ...treinoData, recorrente: e.target.checked })
              }
            />
          </label>
